...
const urlinputTemplate = "./Day%20{daynum}/{typeinput}.js";
const titles = [
  "Day 1: Report Repair",
...
function loadCode(day) {
  const part = document
    ...
    .id.replace(/[^\d]+/g, "");
    const typeinput2 = document
    .querySelector("input[name=tab]:checked")
    .id.match(/sample|input/g).pop();;
  ...
  const html = `<head>
      ...
      <script onerror="notFound()" src="${urlinputTemplate
        .replace("{daynum}", `${day}`.padStart(2, "0"))
        .replace("{typeinput}", typeinput2)}" defer></script>
